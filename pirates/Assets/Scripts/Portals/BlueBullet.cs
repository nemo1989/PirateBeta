...
public class BlueBullet : MonoBehaviour {
	...
	void Start ()
	{
		...
		savepos = player.GetComponent<Move> ();
...
public	void MoveBulletToFaceTheRightDirection(float x, float y)
	...
	void OnTriggerExit2D(Collider2D col){
		...
		if (col.gameObject.tag == "RangeIndicator") {
			Destroy(gameObject);
			PortalWeapon.blueDeployed=false;
			Debug.Log ("am i in blue?");
	...
	void OnTriggerEnter2D(Collider2D col){
		...
		if (col.gameObject.tag == "PortalWall") {
			...
			bluePortal.gameObject.GetComponent<PortalBlue>().whichWayToFace(state);
