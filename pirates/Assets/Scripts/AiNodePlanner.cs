...
using Pathfinding;
public class AiNodePlanner : MonoBehaviour {
    ...
    public void update()
    {
        ...
        if (checkTime >= 7)
        {
            ...
            checkTime = 0;
            random = Random.Range(0, coinHolder.Length);
            if (coinHolder[random] == null)
            {

                return;
            }

            seeker.StartPath(rigi2d.position, coinHolder[random].position, OnPathComplete);
